...
class ProcessThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.update_signal.emit(f"Tempo total de processamento: {elapsed_time:.2f}s")
            self.finished_signal.emit(True, f"Processamento concluído com sucesso! ({elapsed_time:.2f}s)")
